...
const updateFtpCredentials = async () => {
  try {
      ...
      await axios.post(`http://${ipAddress}:8000/api/ftp/upload_credentials`, data, {
          headers: {
...
const startFtpUpload = async () => {
  try {
      ...
      await axios.post(`http://${ipAddress}:8000/api/ftp/upload_folder`, {
...
const checkFtpConnection = async () => {
 try {
    ...
    const response = await axios.get(`http://${ipAddress}:8000/api/ftp/check_connection`);
...
const handleStopUpload = async () => {
  try {
    const response = await axios.post(`http://${ipAddress}:8000/api/ftp/stop_upload`); // Adjust the URL if necessary
...
const validateIp = (ip) => {
 ...
 const handleSave = async () => {
  ...
  try {
      const url = `http://${ipAddress}:8000/api/ip`;  
...
const handleDownload = async () => {
  ...
  const downloadUrl = `http://${ipAddress}:8000/api/download_data`
  ...
  <div className="flex flex-col mb-20">
    <div>
      ...
      <div className="flex-1 flex items-center justify-center overflow-y-auto">
        <div className="grid grid-cols-1 gap-6 p-4 sm:grid-cols-2 lg:grid-cols-3 w-full max-w-[1280px]">

      {/* IP Config Popup */}
      <Popup
        trigger={
          <div className="flex flex-col w-full sm:w-[80%] sm:h-[60%] lg:h-[80%] lg:w-[60%] mx-auto transform rounded-md border-2 transition-transform duration-300 hover:scale-105 border-black">
            <div className="grow px-5 pt-9 pb-20 sm:pb-28 cursor-pointer font-bold text-stone-950">
              IP Config
            </div>
      ...
      <div className="flex flex-col w-full sm:w-[80%] lg:h-[80%] lg:w-[60%] mx-auto transform rounded-md border-2 transition-transform duration-300 hover:scale-105 text-base border border-black border-solid">
          ...
          <img className="w-[30%]" src="./images/graph.png" alt="" />
            ...
            <div className="flex items-center px-5 pb-5">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#5E58E7] to-pink-400" ></span>   
            </div>
      </div>
    ...
    >
      {close => (
        ...
        {/* Log files button */} 
            <div className="flex flex-col w-full sm:w-[80%] lg:h-[80%] lg:w-[60%] mx-auto transform rounded-md border-2 transition-transform duration-300 hover:scale-105 border-black">
                <div className="grow px-5 pt-9 pb-20 sm:pb-28 cursor-pointer font-bold text-stone-950">
                  Log files
                  <img className="w-[20%]" src="./images/png.png" alt="Log Files" />
            </div>
            <div className="flex items-center -mt-12 px-5 pb-5">
        ...
        <Popup
          trigger={
            <div className="flex flex-col w-full sm:w-[80%] lg:h-[80%] lg:w-[60%] mx-auto transform rounded-md border-2 transition-transform duration-300 hover:scale-105 text-base border border-black border-solid">
              <div className="grow px-5 pt-9 pb-20 sm:pb-28 w-full cursor-pointer text-base font-bold text-stone-950">
                Admin
                <img className="w-[20%] mt-4" src="./images/admin.png" alt="Admin" />
              </div>
              <div className="flex items-center  px-5 pb-5">
              </div>
            </div>
